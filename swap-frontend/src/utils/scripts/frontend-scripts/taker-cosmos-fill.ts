...
import { order } from './orders/order-1'
import { ethers } from "ethers";
import { createTimelock } from "./timelock";

const ESCROW_DST_CODE_ID = Number(process.env.COSMOS_ESCROW_DST_CODE_ID);

const taker = process.env.RESOLVER_COSMOS_ADDR;
const MNEMONIC = process.env.RESOLVER_COSMOS_MNEMONIC;
const PREFIX = process.env.PREFIX;
const NATIVE_DENOM = process.env.NATIVE_DENOM;
const RPC_ENDPOINT = process.env.COSMOS_RPC;

async function takerFill() {
    if (order.swap.to.network != 'cosmos') throw new Error('Invalid network for taker escrow')
    ...
    if (account.address != taker) {
        throw new Error('wallet does not match with taker address');
    }
    ...
    const label = 'taker-escrow-order-' + order.orderID;
